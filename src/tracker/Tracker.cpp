...
void Tracker::track(const cv::Mat imagePrev, const cv::Mat img, const std::vector<cv::Point2f>& pointsPrev,
        std::vector<cv::Point2f>& pointsTracked, std::vector<unsigned char>& status)
{
    if (pointsPrev.size() > 0)
    {
        ...
        calcOpticalFlowPyrLK(img, imagePrev, pointsTracked, points_back, status_back, err_back);
